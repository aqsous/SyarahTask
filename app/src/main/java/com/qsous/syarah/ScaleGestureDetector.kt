...
internal class ScaleGestureDetector(private val mListener: OnScaleGestureListener) {
    ...
    private fun setContext(view: View, curr: MotionEvent) {
        ...
        val prev = mPrevEvent
        val prevIndex0 = prev?.findPointerIndex(mActiveId0) ?: -1
        val prevIndex1 = prev?.findPointerIndex(mActiveId1) ?: -1
        ...
        val px0 = prev?.getX(prevIndex0) ?: 0.0f
        val py0 = prev?.getY(prevIndex0) ?: 0.0f
        val px1 = prev?.getX(prevIndex1) ?: 0.0f
        val py1 = prev?.getY(prevIndex1) ?: 0.0f
        ...
        focusX = cx0 + cvx * 0.5f
        focusY = cy0 + cvy * 0.5f
        timeDelta = curr.eventTime - (prev?.eventTime ?: 0L)
        mCurrPressure = curr.getPressure(currIndex0) + curr.getPressure(currIndex1)
        mPrevPressure =
            (prev?.getPressure(prevIndex0) ?: 0.0f) + (prev?.getPressure(prevIndex1) ?: 0.0f)
    }
